 ...
 void
DsdvHeader::Serialize (ns3::Buffer::Iterator i) const
 {
   WriteTo (i, addr_dest);
   i.WriteHtonU32 (0);
 ...
 uint32_t
 DsdvHeader::Deserialize (ns3::Buffer::Iterator start)
 {
...
void DsdvHeader::set_IP(ns3::Ipv4Address a_d){this->addr_dest=a_d;}
 void set_seq( uint32_t seq){this->seq_no_dest=seq;}
  void set_hops(uint32_t hop){ this->no_of_hops=hop;}
  void set_loca(Location lo){9loc(lo);}
  ns3::Ipv4Address get_IP(){return addr_dest;}
             uint32_t get_seq(){return  seq_no_dest; }
             uint32_t get_hops(){return no_of_hops;}
            Location get_loca(){ return loc;}